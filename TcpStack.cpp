...
void TcpStack::open(uint serverPort)
{
	...
	if (DEBUG) { Serial.println("Configuring Ethernet Layer..."); }
	
	MACInit();
	if (DEBUG) { Serial.println("MACInit OK!"); }
	MACInitMacAddr(m_macAddr);
	if (DEBUG) { Serial.println("MACInitMacAddr OK!"); }
	MACOpen();
	if (DEBUG) { Serial.println("MACOpen OK!"); }
	MACEnableRecv();
	if (DEBUG) { Serial.println("MACEnableRecv OK!"); }
